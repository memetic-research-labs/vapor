...
- **Secure auto-pairing** — replace manual auth token copy-paste with a pairing flow that uses a short-lived bootstrap session and pins the trusted extension origin. See issue [#51](https://github.com/memetic-research-labs/comp-tok-stt/issues/51) and [`docs/plan-secure-extension-auto-pairing.md`](plan-secure-extension-auto-pairing.md).
...
- **Browser research adapters** — evolve the bridge from one-shot page capture into site-aware extraction workflows, starting with Amazon review corpora and other structured browser research use cases. See [`docs/plan-browser-research-adapters.md`](plan-browser-research-adapters.md).
